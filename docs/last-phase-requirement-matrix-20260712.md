...
- Physical-device UAT: iPhone workflow is `PASS` for install/launch, orientation, avatars, drafts, activity changes, photo slots, multi-person rejection, historical temporary assessment-path check (superseded on 2026-07-19), categorized recommendations, Thai TTS, filtering, and export. Offline relaunch remains pending. Android physical UAT has not started by user instruction.
