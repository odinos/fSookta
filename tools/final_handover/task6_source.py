...
HIST=[
 ...
 ("H-UAT-007","2026-07-19","","not stated","docs/uat-production-platform-parity-20260719.md","iPhone SE connected/paired but unavailable","iOS simulator + Android emulator","Separate /private/tmp iOS build directory for FileProvider metadata","None - production assessment path","BLOCKED BY DEVICE CONNECTION","Production-path MoveNet/TFLite passed on simulator/emulator; physical iPhone remained blocked"),
]
def historical(staging):
 root=source_root(staging); keys=("evidence_id","date","round_revision","version","evidence_path","device","runner_harness","build_workaround","assessment_bypass","blocker","observation")
 return [{**dict(zip(keys,row)),"sha256":digest(root/row[4]),"evidence_layer":"Historical - not final baseline"} for row in HIST]
...
def defects(staging):
 ...
 raw=[
  ("DEF-HIST-001","Production assessment temporary bypass","c3a0aa0a78216a5dd048c2c80bbbf35042cc8de5","f2262aa68665697d0b51df7dea0b4d63608cfeb6","lib/app/uat_config.dart","Production gate regression and simulator/emulator production-path UAT"),
 ...
 history={x["commit"]:x for x in json.loads((Path(staging)/"working/task6/task6_build_input.json").read_text())["git_history"]}
 d=[{"defect_id":i,"description":desc,"before_commit":b,"after_commit":a,"source_path":p,"fix":f"Corrected in {a}","retest":r,"evidence_path":ep,"sha256":h,"version":"1.3.6+21","status":"Historical correction/retest - not final device evidence","before_git_object":history[b],"after_git_object":history[a]} for i,desc,b,a,p,r in raw]
 ba=[{"change_id":x["defect_id"],"before_commit":x["before_commit"],"after_commit":x["after_commit"],"source_path":x["source_path"],"before":x["description"],"after":x["fix"],"before_changed_paths":x["before_git_object"]["changed_paths"],"after_changed_paths":x["after_git_object"]["changed_paths"],"evidence_path":x["evidence_path"],"sha256":x["sha256"]} for x in d]
 return d,ba
def build_corrected_payload(staging):
 ...
 results=[]
 for t in tests:
  ev=evidence_tuple(t["case_id"],ts,"/Users/kpc/develop/flutter/bin/flutter test --reporter expanded","evidence/flutter_test_1.3.11+28.log",logsha)
  results.append({**t,"requirement_id":"","precondition_input":f"Authoritative baseline {BASELINE}; source-defined fixture in {t['source_path']}","expected":"Source-defined assertions complete without failure","actual":f"Reporter increment +{t['ordinal']} proves the preceding displayed test completed","status":"PASS","baseline":BASELINE,"tester_category":"Automated host/unit/widget runner","timestamp":ts,"method":ev["method"],"raw_path":ev["raw_path"],"sha256":ev["sha256"],"evidence_tuple":ev,"limitations":"Host/unit/widget only; not device/UAT/audio/permission/performance/Firebase/acceptance proof"})
 a=meta["final_commands"]["analyze"]
 ev=evidence_tuple("STATIC-001",a["timestamp"],a["command"],a["path"],a["sha256"])
 results.append({"case_id":"STATIC-001","name":"Final flutter analyze","category":"Static analysis","requirement_id":"","precondition_input":f"Authoritative baseline {BASELINE}","expected":"Analyzer exits zero with no issues","actual":"No issues found","status":"PASS","baseline":BASELINE,"tester_category":"Automated static analysis","timestamp":a["timestamp"],"method":ev["method"],"raw_path":ev["raw_path"],"sha256":ev["sha256"],"evidence_tuple":ev,"limitations":"Static analysis only"})
...
def validate_payload(p,staging):
 ...
 for r in p["result_rows"]:
  if r["status"]=="PASS":
   e=r.get("evidence_tuple",{}); path=Path(staging)/r.get("raw_path","")
   if not required<=set(r) or any(not r.get(k) for k in required-{"requirement_id"}) or not re.fullmatch(r"[0-9a-f]{64}",r.get("sha256","")) or not path.is_file() or digest(path)!=r["sha256"]: raise ValueError("PASS schema")
   if any(e.get(k)!=r.get(k) for k in ("case_id","baseline","timestamp","method","raw_path","sha256")): raise ValueError("PASS normalized join")
