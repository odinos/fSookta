...
export function trainingEvaluationRows(facts) {
  return facts.training_evidence.map(r=>[
    r.evidence_id,r.component,r.dataset_source,r.dataset_path??"N/A with Rationale",r.dataset_status??"N/A with Rationale",
    r.total_samples,r.training_samples,r.holdout_samples,r.unit,r.split_method??"N/A",r.test_size??"N/A",r.random_seed,
    r.xgb_parameters?JSON.stringify(r.xgb_parameters):"N/A",r.preprocessing,r.features,r.labels,r.class_handling,r.selection_criteria,
    r.holdout_risk_accuracy,r.holdout_mae,r.evaluation_boundary,r.raw_metrics_path??"N/A with Rationale",r.raw_metrics_status,r.raw_metrics_note,r.research_trained,r.citation,
  ]);
}
...
async function buildAlgorithm(facts) {
  ...
  matrix(sheets["Training Evaluation"],"Training and Evaluation Evidence","Executable/current evidence separated from historical/template claims",["Evidence ID","Component","Dataset source","Dataset path","Dataset status","Total","Train","Holdout","Unit","Split method","Test size","Seed","XGBRegressor parameters","Preprocessing","Features","Labels","Class handling","Selection criteria","Holdout accuracy","Holdout MAE","Boundary","Raw metrics path","Raw metrics status","Raw metrics note","Research trained","Citation"],trainingEvaluationRows(facts),[24,36,52,54,28,12,12,12,22,30,14,16,80,38,12,60,40,42,18,18,62,54,28,55,18,50]);
...
export function privacyRows(facts) {
  const telemetry = facts.firebase_telemetry;
  const explicitEvents = Object.entries(telemetry.events)
    .map(([name, fields]) => `${name}(${fields.join(", ")})`).join("; ");
  return [
    ["Direct identifiers","name, farmer/participant/profile ID","SharedPreferences/history/export","No assessment backend; user-selected share may leave device","No complete end-to-end schedule in source","Pending Owner Action","Approve lawful basis, access, retention and deletion","lib/app/app_state.dart"],
    ["Participant media linkage","image path, avatar, photo ID/timestamp","Copied to application documents / path persisted","No automatic cloud assessment store","Source does not prove cascading file deletion","Pending Owner Action","Define media custody and erasure verification","lib/core/services/local_image_store.dart"],
    ["Sensitive research/health-adjacent","BMI, MSD placeholders, expert fields, ergonomic risk","Local record/export","Only explicit user share; telemetry separate/default off","Protocol required","Pending Researcher Evidence","Provide consent/ethics/de-identification evidence where applicable","lib/core/services/assessment_export_service.dart"],
    ["Migration backup","sookta.latestBackup","Local pre-migration preference snapshot","N/A with Rationale","No remote disaster recovery","Pending Owner Action","Define backup custody/expiry/restore test","lib/app/app_state.dart"],
    ["Remote database/server retention","N/A","No remote assessment database found","N/A with Rationale","N/A until introduced","N/A with Rationale","Reassess before cloud/API addition","03_API_Applicability_Statement.docx"],
    ["Firebase telemetry",`Default-off explicit events: ${explicitEvents}; Crashlytics receives sanitized explicit-event context`,"Separate from local assessment persistence; SOOKTA_TELEMETRY_ENABLED defaults false",`${telemetry.log_app_open ? "logAppOpen; " : ""}${telemetry.analytics_observer_navigation ? "SDK-generated navigation/screen analytics via FirebaseAnalyticsObserver; " : ""}explicit Analytics/Crashlytics only when enabled`,"Firebase project/privacy/store disclosure and remote retention are owner-controlled and not proven by source","Pending Owner Action","Approve configuration, project custody, disclosure, SDK behavior, and retention","lib/core/services/firebase_telemetry_service.dart; lib/screens/main/evaluation_form_screen.dart; lib/app/sookta_app.dart"],
  ];
}
...
async function buildData(facts) {
  ...
  matrix(sheets["Privacy Retention"],"Privacy, Retention, Backup, and Deletion","Source behavior separated from owner/research protocol",["Data class","Examples","Local behavior","Remote behavior","Retention/deletion","Status","Owner action","Evidence"],privacyRows(facts),[34,48,58,45,50,30,58,55]);
