...
function syntheticExample(field, index) {
...
export function dataDictionaryRows(facts) {
  return facts.all_history_csv_headers.map((field, index) => [
    field,
    `Ordered all-history export field ${index + 1}; meaning follows source calculation/serialization.`,
    inferType(field), "Yes where source value is unavailable", "See Enumerations Values / source", /weight/i.test(field) ? "kg" : /cost|impact|saved/i.test(field) ? "THB" : /distance/i.test(field) ? "m or source-labeled unit" : "N/A",
    "Assessment/profile/history/export service", "Direct, mapped, or derived as documented in Derivations",
    "Blank or '-' according to source path", inferPrivacy(field), "SharedPreferences history/profile or derived at export",
    `${index + 1}: ${field}`, syntheticExample(field, index), "Source validation; preserve blank/fallback behavior",
    VERSION, "lib/core/services/assessment_export_service.dart",
  ]);
}
...
function recommendationTriggerRows() {
  const activities=["transplanting","fertilizing","pesticide","pruning","harvesting","transport"];
  const tiers=["low","medium","high","veryHigh"];
  const rows=[]; for(const activity of activities) for(const tier of tiers) rows.push([activity,tier,`act_${activity}_${tier}`,"Activity recommendation key plus manual-handling reference when applicable","lib/core/services/risk_recommendation_service.dart"]); return rows;
}
...
async function buildAlgorithm(facts) {
  ...
  matrix(sheets["Model Algorithm Inventory"],"Model and Algorithm Inventory","Pretrained, project-trained, deterministic, template, advisory and legacy roles",["Role ID","Component","Identifier","Binary SHA-256","Runtime path","Input","Output","Training class","Authority","Fallback","Limitations","Citation"],facts.model_algorithm_inventory.map(r=>[r.role_id,r.component,r.identifier,r.binary_sha256,r.runtime_path,r.input,r.output,r.training_class,r.authority,r.fallback,r.limitations,r.citation]),[20,27,38,34,35,34,28,28,25,38,55,48]);
  ...
  matrix(sheets["Detected Risk Mapping"],"Detected-Risk Mapping","Deterministic result authority and optional advisory attachment",["Source","Input tier/value","Mapped output","Priority","Conflict rule","Evidence"],[
    ["REBA","1-3 / 4-7 / 8-10 / 11-15","Low / Medium / High / Very High",1,"Primary","lib/core/services/ergo_calculator.dart"],["Lifting index","<=1 / <=3 / >3","Low / Medium / High",1,"Applicable ISO risk combined by higher tier","lib/core/services/ergo_calculator.dart"],["Push/pull ratio","<0.8 / 0.8..1 / >1","Low / Medium / High",1,"Applicable ISO risk combined by higher tier","lib/core/services/ergo_calculator.dart"],["XGBoost","probability/risk signal","Advisory alert",2,"Cannot lower/replace REBA/ISO","assets/models/model_artifact_manifest.json"],["Daily logistic","watch .45 / high .65 / critical .82","Follow-up template tier",3,"Separate from posture scoring","assets/ml/daily_injury_logistic_model.json"]
  ],[24,34,35,15,48,55]);
  matrix(sheets["Recommendation Triggers"],"Recommendation Triggers","Activity and risk-key mapping",["Activity","Tier","Source key pattern","Behavior","Evidence"],recommendationTriggerRows(),[24,18,38,55,52]);
...
export function persistedRows(facts) {
  const classify=(name)=>/name|farmer|profile|location|age|gender|weight|height|income|avatar|photo|image/i.test(name)?"Sensitive participant/profile data":/risk|score|msd|expert|economic|bmi|pose|joint/i.test(name)?"Sensitive research/health-adjacent":"Operational metadata";
  const example=(name)=>/id/i.test(name)?"SYN-001":/date|time|created|saved/i.test(name)?"2026-08-24T09:00:00+07:00":/score|weight|height|force|distance|duration|count|ratio|percent/i.test(name)?"1":"synthetic_value";
  const make=(name,description,type,location,privacy,lifecycle,compatibility,validation,evidence)=>[name,description,type,"Source-defined; nullable unless constructor/source requires value","Source-defined enum/range; see evidence","Source-defined",location,"Serialized by source toJson / SharedPreferences path","Blank/null or source fallback",privacy,location,"N/A - persisted field, not a CSV-order assertion",example(name),validation,VERSION,evidence];
  const keys=facts.preference_keys.map((key,index)=>make(key,"SharedPreferences top-level key",key.includes("Version")?"int":key.includes("Completed")?"bool":"JSON/string or source-defined scalar",`SharedPreferences: ${key}`,classify(key),key.includes("latestBackup")?"Pre-migration local snapshot":"Persisted current state",index===facts.preference_keys.length-1?"Schema-specific":"Current source schema","Restore/migrate in app_state.dart; fail visibly on malformed state","lib/app/app_state.dart"));
  const records=(facts.persisted_record_fields??[]).map(({field,source})=>make(field,"Nested persisted record field","Source-defined scalar/object/list","JSON record under SharedPreferences keys",classify(field),"Persisted with owning profile, draft, history, pose, or assessment record","Current source schema","fromJson/toJson and source defaults; malformed restore fails visibly",source));
  return [...keys,...records];
}
