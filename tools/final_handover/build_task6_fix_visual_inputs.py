...
dirs=[ROOT/'renders/task6/round2_algorithm',ROOT/'renders/task6/round2_uat_doc',ROOT/'renders/task6/round2_pdf_audit',ROOT/'renders/task6/round2_pdf_master',ROOT/'renders/task6/round2_pdf_uat']+sorted((ROOT/'renders/task6/round2_sheets').iterdir())
images=[]
for d in dirs:
...
manifest=ROOT/'manifests/task6_expected_visual_renders.json'; manifest.write_text(json.dumps({'schema_version':2,'count':len(rows),'renders':rows},indent=2)+'\n')
out=ROOT/'renders/task6/round2_contact_sheets'; out.mkdir(parents=True,exist_ok=True)
...
(ROOT/'manifests/task6_round2_contact_sheets.json').write_text(json.dumps({'count':len(sheets),'sheets':sheets},indent=2)+'\n')
